    ...
    if (!app.isPackaged) {
      ...
      try {
        ...
        win.loadFile(path.join(__dirname, '../../build/index.html')).catch(err => {
          ...
          win.webContents.loadFile(path.join(__dirname, '../renderer/error.html'));
        });
      }
    } else {
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('create-collection', async (event, collectionData) => {
    try {
      ...
      // Create a proper collection object with name property
      const collection = await storage.createCollection({ name: collectionName });
